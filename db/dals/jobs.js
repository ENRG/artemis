...
, schema: {
  ...
  , company:          { type: 'text' }
  , ambient:          { type: 'double precision' }
  , allowable_day:    { type: 'double precision' }
  , allowable_night:  { type: 'double precision' }
  , threshold_1:      { type: 'double precision' }
  , threshold_2:      { type: 'double precision' }
  , threshold_3:      { type: 'double precision' }
  , is_active:        { type: 'boolean' }
  , nmt_id:           { type: 'int', references: { table: 'nmts' } }
  , timezone_offset:  { type: 'int',        default: 0 }
  , start_date:       { type: 'timestamp',  default: 'now()' }
  , end_date:         { type: 'timestamp',  default: 'now()' }
...
, createHourlyReport: function(jobId, callback){
    var createReport = function(callback){
      ...
      db[ ['job', jobId, 'leqs'].join('_') ].find($query, function(error, results){
