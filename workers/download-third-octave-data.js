/**
 * Worker: Download 3rd Octave Files
 */
...
var onNmtLoaded = function( nmt, callback ){
  ...
  var onFile = function( file, callback ){
    ...
    db.nmt_hourly_logs.findOne( $query, function( error, log ){
      ...
      var p = path.join( config.nmt.remoteThirdOctavePath, file.name );

      ftp.get( p, function( error, fstream ){
        if ( error ) return callback( error );

        var loc = config.nmt.localThirdOctavePath.replace( ':nmtId', nmt.id );
  ...
  ftp.on( 'ready', function(){
    var p = path.join( config.nmt.remoteThirdOctavePath, '*.dat' );
